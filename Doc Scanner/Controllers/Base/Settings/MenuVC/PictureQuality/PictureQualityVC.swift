...
class PictureQualityVC: UIViewController {
    ...
    @IBOutlet var qualityViews   : [UIView]!
...
extension PictureQualityVC {
    func initVC(){
        ...
        ///Outlet Collection Corner Radius Setup
        for view in qualityViews {
            view.layer.cornerRadius = view.frame.height / 2
        }
        
        /// Setup Selected Button
        buttonLabels.forEach { (label) in
            if label.accessibilityIdentifier == Constants.sharedInstance.pictureQuality {
...
extension PictureQualityVC {
    ...
    @IBAction func didTapGetPro(_ sender: UIButton) {
        ...
        premiumVC.fromVC                 = .pictureQualityVC
    ...
    @IBAction func togglePictureQuality(_ sender: UIButton) {
        buttonLabels.forEach { (label) in
        ...
        Constants.sharedInstance.pictureQuality = sender.accessibilityIdentifier ?? "medium"
    }
}
