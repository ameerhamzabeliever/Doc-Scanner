...
public protocol ImageScannerControllerDelegate: NSObjectProtocol {
    ...
    /// Tells the delegate that the user has scanned the data and now open the document view
    ///
    /// - Parameters:
    ///   - scanner: The scanner controller object managing the scanning interface.
...
public final class ImageScannerController: UINavigationController {
    ...
    public required init(
        ...
        if dataSource != nil {
            scannerVC.datasource  = dataSource!
            scannerVC.isEditOrAdd = isEditOrAdd
            if let editFile = document as? DocumentsModel {
                scannerVC.editFile = editFile
            }
        }
        ...
        /// If an image was passed in by the host app (e.g. picked from the photo library),
        /// use it instead of the document scanner.
        if let image = image {
