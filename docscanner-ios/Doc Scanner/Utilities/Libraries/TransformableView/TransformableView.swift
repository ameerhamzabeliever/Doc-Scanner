...
enum TransformableViewHandler:Int {
    case close = 0
    case rotate
    case flip
}

enum TransformableViewPosition:Int {
    case topLeft = 0
    case topRight
    case bottomLeft
    case bottomRight
...
@objc protocol TransformableViewDelegate {
    @objc func transformableViewDidBeginMoving(_ transformableView: TransformableView)
    @objc func transformableViewDidChangeMoving(_ transformableView: TransformableView)
    @objc func transformableViewDidEndMoving(_ transformableView: TransformableView)
    @objc func transformableViewDidBeginRotating(_ transformableView: TransformableView)
    @objc func transformableViewDidChangeRotating(_ transformableView: TransformableView)
    @objc func transformableViewDidEndRotating(_ transformableView: TransformableView)
    @objc func transformableViewDidClose(_ transformableView: TransformableView)
    @objc func transformableViewDidTap(_ transformableView: TransformableView)
}

class TransformableView: UIView {
    var delegate: TransformableViewDelegate!
     ...
     */
    func setImage(_ image:UIImage, forHandler handler: TransformableViewHandler) {
        switch handler {
        case .close:
     ...
     *  @param handler  The editing handler.
     */
    func setPosition(_ position: TransformableViewPosition, forHandler handler: TransformableViewHandler) {
    ...
    func setHandlerSize(_ size:Int) {
        ...
        self.setPosition(TransformableViewPosition(rawValue: self.closeImageView.tag)!, forHandler: .close)
        self.rotateImageView.frame = handlerFrame
        self.setPosition(TransformableViewPosition(rawValue: self.rotateImageView.tag)!, forHandler: .rotate)
        self.flipImageView.frame = handlerFrame
        self.setPosition(TransformableViewPosition(rawValue: self.flipImageView.tag)!, forHandler: .flip)
    ...
    func handleMoveGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if let delegate = self.delegate {
                delegate.transformableViewDidBeginMoving(self)
            }
        case .changed:
            ...
            if let delegate = self.delegate {
                delegate.transformableViewDidChangeMoving(self)
            }
        case .ended:
            self.center = CGPoint(x: self.beginningCenter.x + (touchLocation.x - self.beginningPoint.x), y: self.beginningCenter.y + (touchLocation.y - self.beginningPoint.y))
            if let delegate = self.delegate {
                delegate.transformableViewDidEndMoving(self)
            }
        default:
            break
        }
    ...
    func handleRotateGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if let delegate = self.delegate {
                delegate.transformableViewDidBeginRotating(self)
            }
        case .changed:
            ...
            if let delegate = self.delegate {
                delegate.transformableViewDidChangeRotating(self)
            }
        case .ended:
            if let delegate = self.delegate {
                delegate.transformableViewDidEndRotating(self)
            }
        default:
            break
        }
    ...
    func handleCloseGesture(_ recognizer: UITapGestureRecognizer) {
        if let delegate = self.delegate {
            delegate.transformableViewDidClose(self)
        }
    ...
    func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        if let delegate = self.delegate {
            delegate.transformableViewDidTap(self)
        }
    }
